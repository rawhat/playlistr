...
class Playlist {
    ...
    getCurrentPlaytime() {
        return this.isPaused
            ? this.currentTime
            : this.currentTime + (Date.now() - this.startDate) / 1000;
    }

    async addSong(song) {
        if (this.driver && this.conn) {
            try {
                let params = Object.assign({}, _.omit(song, 'driver'), {
                    addedAt: Date.now(),
                    title: this.title,
                });
    ...
    async removeSong(song) {
        if (this.driver) {
            ...
            try {
                await session.run(
                    'MATCH (p:Playlist)-[r:HAS]-(s:Song) WHERE p.title = {title} AND s.url = {url} DELETE r, s;',
        ...
        if (this.driver) {
            ...
            try {
                let results = await session.run(
                    'MATCH (p:Playlist)-[:HAS]-(s:Song) WHERE p.title = {title} RETURN s as song;',
                    { title: this.title }
                );
...
class PlaylistManager {
    ...
    addExistingPlaylist(playlist) {
        ...
        this.playlists[playlist.title] = new Playlist(
            title,
            category,
            password,
            openSubmissions,
...
var convertTime = function(timeString) {
    ...
    var segmentInts = _.map(segments, function(segment) {
